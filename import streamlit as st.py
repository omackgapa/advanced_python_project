...
def get_puuid(game_name, tag_line):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"PUUID 조회 오류 (코드: {response.status_code})")
            return None
...
if page == "🛡️ 숙련도/랭크 조회하기":
    ...
    # (수정) st.form과 submit_button 제거
    full_riot_id = st.text_input("Riot ID (이름#태그):", 
                                 value="hide on bush#KR1", 
                                 help="Riot ID를 '이름#태그' 형식으로 입력하세요.")

    # (수정) if submit_button... 대신 if full_riot_id... 로 변경
    if full_riot_id:
        try:
